...
- `sendReminderEmail()` with HTML template - subject: "⏰ Overdue: {task}", from: `onboarding@resend.dev`
...
- `POST /api/action-items` - create action item linked to meeting (ownership verified)
...
### Added
- AI meeting analysis via Google Groq API (LLaMA 3) with `responseMimeType: 'application/json'`
- `POST /api/meetings/:id/analyze` - analyzes transcript and returns summary, actionItems, decisions, followUpSuggestions
...
- Conflict detection - returns 409 if analysis already exists for a meeting

## [1.0.0] - 2026-06-05

### Added
- Project foundation - Express 4.18.2 + TypeScript 5.x + PostgreSQL 15 + Prisma 5.x
- Prisma schema with 5 models: User, Meeting, MeetingAnalysis, ActionItem, ReminderLog
...
- `GET /api/meetings` - list meetings with pagination and date range filtering
- `GET /api/meetings/:id` - get meeting with analysis and action items
